...
// node_modules/maplibre-gl-video-export/src/utils.js
...
function catmullRomPoint(p0, p1, p2, p3, t, tension = 0.5) {
  ...
  const lng = (2 * p1[0] - 2 * p2[0] + v0 + v1) * t3 + (-3 * p1[0] + 3 * p2[0] - 2 * v0 - v1) * t2 + v0 * t + p1[0];
  const w0 = (p2[1] - p0[1]) * tension;
  const w1 = (p3[1] - p1[1]) * tension;
  const lat = (2 * p1[1] - 2 * p2[1] + w0 + w1) * t3 + (-3 * p1[1] + 3 * p2[1] - 2 * w0 - w1) * t2 + w0 * t + p1[1];
  return [lng, lat];
}
function resamplePath(coords, targetSpacingKm = 0.01) {
  if (!coords || coords.length < 2) return coords;
  const resampled = [coords[0]];
...
function resamplePathCatmullRom(coords, targetSpacingKm = 0.01, tension = 0.3) {
  if (!coords || coords.length < 2) return coords;
  if (coords.length === 2) return resamplePath(coords, targetSpacingKm);
  const smoothCurve = [];
...
// node_modules/maplibre-gl-video-export/src/animations.js
var sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
var ROAD_QUERY_FILTER = [
  "all",
...
var normalizeBearingDiff = (diff) => {
  while (diff > 180) diff -= 360;
  while (diff < -180) diff += 360;
  return diff;
};
var isValidCoordinate = (coord) => {
  return coord && Array.isArray(coord) && typeof coord[0] === "number" && typeof coord[1] === "number";
};
var cleanupMap2AndDebugLayer = (options, map) => {
  if (options.map2) {
    try {
      options.map2.remove();
    } catch (e) {
...
var segmentIntersection = (p1, p2, p3, p4) => {
  const x1 = p1[0];
  const y1 = p1[1];
  const x2 = p2[0];
  const y2 = p2[1];
...
var AnimationConstraints = class {
  constructor(options = {}) {
    this.maxBounds = options.maxBounds || null;
    this.minZoom = options.minZoom !== void 0 ? options.minZoom : null;
    this.maxZoom = options.maxZoom !== void 0 ? options.maxZoom : null;
  ...
  applyCameraConstraints(options) {
    ...
    if (options.center !== void 0 && options.center !== null) {
      constrained.center = this.constrainCenter(options.center);
    }
    if (options.zoom !== void 0 && options.zoom !== null) {
...
async function flyToWaypoint(map, waypoint, transitionDuration, { checkAbort, updateStatus } = {}) {
  ...
  if (waypoint.zoom !== void 0) flyToOptions.zoom = waypoint.zoom;
  if (waypoint.bearing !== void 0) flyToOptions.bearing = waypoint.bearing;
  if (waypoint.pitch !== void 0) flyToOptions.pitch = waypoint.pitch;
  if (transitionDuration === 0 || transitionDuration < 10) {
    const jumpToOptions = { center: waypoint.center };
    if (waypoint.zoom !== void 0) jumpToOptions.zoom = waypoint.zoom;
    if (waypoint.bearing !== void 0) jumpToOptions.bearing = waypoint.bearing;
    if (waypoint.pitch !== void 0) jumpToOptions.pitch = waypoint.pitch;
...
var rotatePanorama360 = async (map, duration, { checkAbort, degreesPerStep = 2, pitch, onStep } = {}) => {
  ...
  const totalSteps = 360 / degreesPerStep;
  ...
  for (let i = 0; i < totalSteps; i++) {
    ...
    if (pitch !== void 0) {
      if (typeof pitch === "number") {
        currentPitch = pitch;
      } else if (pitch.from !== void 0 && pitch.to !== void 0) {
    ...
    if (map.getTerrain && map.getTerrain()) {
      const pitchToCheck = easToOptions.pitch !== void 0 ? easToOptions.pitch : map.getPitch();
      if (pitchToCheck > 0) {
...
var capabilitiesCache = /* @__PURE__ */ new WeakMap();
function calculateTerrainAwareZoomAtPoint(map, center, pitch = 60) {
  const defaultZoom = 3;
  if (!map.getTerrain || !map.getTerrain()) {
    return defaultZoom;
  }
  const centerPoint = Array.isArray(center) ? { lng: center[0], lat: center[1] } : center;
  const baseDistanceDegrees = 0.01;
  const viewDistanceFactor = 1 + pitch / 85 * 9;
  const baseRadii = [0.25, 0.5, 0.85, 1.3];
  const radii = baseRadii.map((r) => r * baseDistanceDegrees * viewDistanceFactor);
  const directions = 16;
  const samplePoints = [centerPoint];
  ...
  const elevationKm = maxElevation / 1e3;
...
async function terrainAwareEaseTo(map, options, checkAbort) {
  if (map.getTerrain && map.getTerrain() && options.pitch > 0) {
    ...
    if (options.zoom !== void 0 && options.zoom < terrainAwareZoom) {
...
var AnimationDirector = class {
  ...
  _detectCapabilities(forceDetect = false) {
    ...
    const caps = {
      ...
      // Terrain source (raster-dem) is available
      hasTerrain: false,
      // Terrain is currently enabled on the map
      terrainSourceId: null,
      // ID of the terrain source (if available)
      hasHillshade: false,
    ...
    const sources = (style == null ? void 0 : style.sources) || {};
    ...
    if (style == null ? void 0 : style.glyphs) {
      caps.hasGlyphs = true;
    }
    if (style == null ? void 0 : style.sprite) {
      caps.hasSprites = true;
    }
    const sourceLayers = /* @__PURE__ */ new Set();
    const layers = (style == null ? void 0 : style.layers) || [];
    ...
    const styleUrl = (style == null ? void 0 : style.sprite) || "";
  ...
  static async _positionHelperMapAhead(map2, currentPos, bearing, searchRadius) {
    try {
      ...
      map2.fitBounds([[minLng, minLat], [maxLng, maxLat]], {
        ...
        // No animation
        padding: 0,
        // No padding needed for invisible map
        duration: 0
        // Instant
      });
  ...
  createAdaptiveAnimation(control, options = {}) {
    ...
    return {
      setup: null,
      // No setup needed
      animation: async (map, control2) => {
        const { updateStatus, checkAbort } = control2;
        ...
        for (const animation of animations) {
          await animation(control2, timePerAnimation);
          checkAbort();
        }
  ...
  try {
    ...
    if (uniqueSources.size === 0) {
      ...
      console.warn("[HelperMap] Available source-layers:", Array.from(new Set(
        (style.layers || []).filter((l) => l["source-layer"]).map((l) => l["source-layer"])
      )));
      return null;
    }
    ...
    uniqueSources.forEach((sourceId) => {
      ...
      if (vectorSource && vectorSource.type === "vector") {
        minimalSources[sourceId] = {
          ...
          ...vectorSource.minzoom !== void 0 && { minzoom: vectorSource.minzoom },
          ...vectorSource.maxzoom !== void 0 && { maxzoom: vectorSource.maxzoom },
    ...
    const minimalStyle = {
      ...
      // Minimal invisible layers to force feature loading
      glyphs: style.glyphs,
      sprite: style.sprite,
    ...
    return {
      vectorSources: caps.vectorSources,
      // Return all source/layer mappings
      style: minimalStyle
    };
  } catch (error) {
...
function _findNearbyRoadInCardinalDirections(fromPoint, currentBearing, usedSegmentIds, roads2, options = {}) {
  var _a, _b;
  const { prefer = null, searchRadius = 2e-3 } = options;
  const preferredClasses = prefer ? Array.isArray(prefer) ? prefer : [prefer] : [];
  console.log("[RoadSearch] Searching nearby roads in cardinal directions..." + (preferredClasses.length ? ` (prefer: ${preferredClasses.join(", ")})` : ""));
  ...
  for (const direction of searchDirections) {
    ...
    for (const road of roads2) {
      ...
      if (minDist > searchRadiusKm) continue;
      const bearingDiff = Math.abs(normalizeBearingDiff(direction - currentBearing));
      let score = minDist + bearingDiff / 180 * 1e-3;
      const roadClass = ((_a = road.properties) == null ? void 0 : _a.class) || "unknown";
      ...
      if (score < bestScore) {
        ...
        bestRoad = {
          ...
          distance: actualDist,
          // Store ACTUAL distance from current position
          direction,
          bearingDiff
        };
      }
  ...
  const maxJumpDistanceKm = 0.25;
  if (bestRoad && bestRoad.distance > maxJumpDistanceKm) {
    console.log(`[RoadSearch] ⚠️ Found road but too far (${(bestRoad.distance * 1e3).toFixed(0)}m > 250m) - rejecting to avoid huge jump`);
    bestRoad = null;
  }
  if (bestRoad) {
    const roadClass = ((_b = bestRoad.road.properties) == null ? void 0 : _b.class) || "unknown";
    const isPreferred = preferredClasses.includes(roadClass);
    console.log(`[RoadSearch] 🔍 Found ${isPreferred ? "✨ preferred " : ""}${roadClass} at ${bestRoad.direction}° (${(bestRoad.distance * 1e3).toFixed(0)}m away, bearing Δ${bestRoad.bearingDiff.toFixed(1)}°)`);
  } else {
    console.log(`[RoadSearch] No roads found within ${(searchRadiusKm * 1e3).toFixed(0)}m in any direction`);
  }
  return bestRoad;
}
var PresetAnimations = {
  /**
     * Simple 360 orbit
     */
  ...
  figure8: async (map, { updateStatus, checkAbort }, options = {}) => {
    const duration = options.duration || 15e3;
    const waypoints = options.waypoints || null;
    if (waypoints) {
      const optimalView = getOptimalViewForWaypoints(map, waypoints);
      if (optimalView) {
  ...
  spiralZoom: async (map, { updateStatus, checkAbort }, options = {}) => {
    const duration = options.duration || 12e3;
  ...
  neighborhood: async (map, { updateStatus, checkAbort }, options = {}) => {
    const duration = options.duration || 25e3;
  ...
  propertyShowcase: async (map, { updateStatus, checkAbort }, options = {}) => {
    const duration = options.duration || 2e4;
  ...
  panorama: async (map, { updateStatus, checkAbort }, options = {}) => {
    ...
    await rotatePanorama360(map, duration, {
      ...
      onStep: (currentBearing, progress) => {
        const pitchCurve = progress < 0.5 ? startPitch + (50 - startPitch) * (progress * 2) : 50 - (50 - startPitch) * ((progress - 0.5) * 2);
  ...
  aerialSweep: async (map, { updateStatus, checkAbort }, options = {}) => {
    ...
    await rotatePanorama360(map, duration * 0.75, {
      ...
      updateStatus,
      pitch: 75,
      // Fixed pitch during first 85% of rotation
      onStep: (currentBearing, progress) => {
        if (progress >= 0.85) {
  ...
  droneShot: async (map, { updateStatus, checkAbort }, options = {}) => {
    ...
    const ascentSteps = 90;
    const ascentDuration = duration * 0.3;
    ...
    await rotatePanorama360(map, duration * 0.4, {
      checkAbort,
      updateStatus,
      pitch: 65
    });
    ...
    const descentDuration = duration * 0.3;
  ...
  orbitZoom: async (map, { updateStatus, checkAbort }, options = {}) => {
    ...
    await rotatePanorama360(map, duration, {
      ...
      updateStatus,
      pitch: 45,
      // Moderate tilt for dramatic effect
  ...
  waveMotion: async (map, { updateStatus, checkAbort }, options = {}) => {
    const duration = options.duration || 18e3;
  ...
  butterfly: async (map, { updateStatus, checkAbort }, options = {}) => {
    ...
    for (const point of points) {
      map.flyTo({
        ...
        essential: true,
        easing: (t) => t
        // Linear for smooth continuous motion
      });
      await map.once("moveend");
      checkAbort();
    }
  ...
  terrainFollowing: async (map, { updateStatus, checkAbort }, options = {}) => {
    ...
    const msPerStep = duration * 0.95 / steps;
    const zoomBuffer = [];
    const bufferSize = 5;
    ...
    for (let step = 0; step < steps; step++) {
      ...
      if (centerElevation !== null && centerElevation >= 0) {
        const elevationKm = centerElevation / 1e3;
        const baseZoom = 17;
  ...
  _followPathWithVehicleSetup: (map, control, options = {}, vehicleProfile) => {
    ...
    let map2 = null;
    let div2 = null;
    let sourceId2 = "openmaptiles";
    let sourceLayer2 = "transportation";
    const debugFeatures = [];
    return {
      setup: async (map3, control2, { updateStatus, checkAbort }) => {
        var _a, _b;
        console.log("[HelperMap] Creating invisible query map...");
        try {
          const styleInfo = _extractMinimalStyle(map3);
          if (styleInfo && styleInfo.vectorSources.roads.sourceId) {
            ...
            const mainContainer = map3.getContainer();
            ...
            map2 = new maplibregl.Map({
              ...
              zoom: 15,
              // Optimal zoom for vector tile data (14-18 range)
            ...
            for (let i = 1; i < coords.length; i++) {
              ...
              if (dist !== null && dist < minIntersectionDistance) {
                ...
                closestIntersection = {
                  ...
                  direction: dir.name,
                  distance: dist,
                  roadClass: ((_a = road.properties) == null ? void 0 : _a.class) || "unknown"
                };
              }
            }
          }
        ...
        if (!closestIntersection) {
          ...
          closestIntersection = {
            ...
            roadClass: ((_b = closestRoad2.properties) == null ? void 0 : _b.class) || "unknown"
          };
        }
        const closestRoad = closestIntersection.road;
        const detectedClass = closestIntersection.roadClass;
        console.log(`[Setup] Found road by intersection: ${detectedClass} at ${(closestIntersection.distance * 111e3).toFixed(1)}m (direction: ${closestIntersection.direction})`);
        ...
        if (roadCoords.length >= 2) {
          const [firstLng2, firstLat2] = roadCoords[0];
          const [lastLng, lastLat] = roadCoords[roadCoords.length - 1];
          const roadBearing = calculateBearing(firstLng2, firstLat2, lastLng, lastLat);
        ...
        map3.easeTo({ pitch: targetPitch, duration: 1e3, essential: true });
        await map3.once("moveend");
        checkAbort();
      ...
      animation: async (map3, control2) => {
        ...
        await PresetAnimations._followPathWithVehicle(map3, control2, animOptions, vehicleProfile);
      },
  ...
  _followPathWithVehicle: async (map, { updateStatus, checkAbort }, options = {}, vehicleProfile) => {
    ...
    let roadCoords = closestRoad.geometry.coordinates;
    const roadClass = ((_a = closestRoad.properties) == null ? void 0 : _a.class) || "road";
    if (roadCoords.length >= 2) {
      const [firstLng, firstLat] = roadCoords[0];
      const [lastLng, lastLat] = roadCoords[roadCoords.length - 1];
    ...
    const findNextSegment = async (lastPoint, secondLastPoint, usedIds) => {
      ...
      const connectionThreshold = 5e-4;
      ...
      console.log(`[RoadChain] Connection threshold: ${(connectionThreshold * 111e3).toFixed(0)}m`);
      for (const road of currentRoads2) {
        ...
        const roadName = (_a2 = road.properties) == null ? void 0 : _a2.name;
        const roadRef = (_b2 = road.properties) == null ? void 0 : _b2.ref;
        const roadClass2 = (_c2 = road.properties) == null ? void 0 : _c2.class;
        ...
        const isSameClass = roadClass2 && currentRoadClass && roadClass2 === currentRoadClass;
        let score = 0;
        if (isSameRef) {
          score = 0 + distance * 10 + bearingDiff * 0.01;
        } else if (isSameName) {
          score = 100 + distance * 10 + bearingDiff * 0.01;
        } else if (isSameClass) {
          score = 1e3 + distance * 10 + bearingDiff * 50;
        } else {
          score = 1e4 + distance * 100 + bearingDiff * 200;
        }
        candidateCount++;
        const candidateLabel = roadRef ? `[${roadRef}]` : roadName || roadClass2;
        const priorityLabel = isSameRef ? "P1-SameRef" : isSameName ? "P2-SameName" : isSameClass ? "P3-SameClass" : "P4-Different";
        console.log(`[RoadChain]   Candidate #${candidateCount}: ${candidateLabel} (${priorityLabel}) bearing Δ${bearingDiff.toFixed(1)}°, dist ${(distance * 111e3).toFixed(1)}m, score ${score.toFixed(1)}`);
        if (score < bestScore) {
          bestScore = score;
          bestNextSegment = {
            road,
      ...
      if (!bestNextSegment && options.map2 && vehicleProfile.searchRadius) {
        ...
        const zoomsToTry = currentZoom2 === 18 ? [16, 17] : [];
              ...
              if (effectiveCoords.length >= 2) {
                ...
                if (bearingDiff <= 150) {
                  ...
                  bestNextSegment = {
                    ...
                    bearingDiff,
                    distance: minDist,
                    score: 1e3 + bearingDiff * 50,
                    // Simple score
                    roadName: (_d2 = road.properties) == null ? void 0 : _d2.name,
                    roadRef: (_e2 = road.properties) == null ? void 0 : _e2.ref
                  };
                  break;
                }
              }
    ...
    const targetPitch = vehicleProfile.pitch;
    map.easeTo({ pitch: targetPitch, duration: 1e3, essential: true });
    await map.once("moveend");
    checkAbort();
    const vehicleSpeedKmh = vehicleProfile.speedKmh || 30;
    const maxSegments = 1e4;
    console.log(`[RoadFollow] Vehicle speed: ${vehicleSpeedKmh} km/h`);
    const startTime = maplibregl.now();
    let currentSegmentCoords = vehicleProfile.smoothPath ? resamplePathCatmullRom(roadCoords, 0.01) : resamplePath(roadCoords, 0.01);
    currentSegmentCoords.roadClass = (_b = closestRoad.properties) == null ? void 0 : _b.class;
    currentSegmentCoords.roadName = (_c = closestRoad.properties) == null ? void 0 : _c.name;
    currentSegmentCoords.roadRef = (_d = closestRoad.properties) == null ? void 0 : _d.ref;
    const zoomBuffer = [];
    const bufferSize = vehicleProfile.smoothing;
    updateStatus(`${vehicleProfile.icon} Following road network...`);
    const usedSegmentIds = /* @__PURE__ */ new Set([closestRoad.id]);
    let currentSegmentIndex = 1;
    let segmentCount = 1;
    let totalPointsVisited = 1;
    try {
      while (true) {
        checkAbort();
        ...
        if (!maplibregl.isTimeFrozen || !maplibregl.isTimeFrozen()) {
          if (elapsed >= duration) {
            console.log(`[RoadChain] Animation complete: ${(elapsed / 1e3).toFixed(1)}s, ${totalPointsVisited} points, ${segmentCount} segments`);
            break;
          }
        }
        if (currentSegmentIndex >= currentSegmentCoords.length) {
          ...
          if (!nextSegment) {
            ...
            const currentClass = currentSegmentCoords.roadClass || ((_e = closestRoad.properties) == null ? void 0 : _e.class);
            const prefer = currentClass ? [currentClass] : [];
            const cardinalSearchRadius = Math.min(2e-3, vehicleProfile.searchRadius || 2e-3);
          ...
          if (!nextSegment && vehicleProfile.supportsExploration) {
            ...
            for (let step = 1; step <= maxSteps && !foundRoad; step++) {
              ...
              const currentClass = currentSegmentCoords.roadClass || ((_f = closestRoad.properties) == null ? void 0 : _f.class);
              foundRoad = _findNearbyRoadInCardinalDirections(
                ...
                roads2,
                {
                  ...
                  searchRadius: (vehicleProfile.searchRadius || 2e-3) * 0.5
                  // Half radius for exploration mode
                }
              );
              if (foundRoad) {
                ...
                for (let i = 1; i <= step; i++) {
                  const lng2 = lastPoint[0] + stepDistance * i * Math.sin(radians);
                  const lat2 = lastPoint[1] + stepDistance * i * Math.cos(radians);
                  intermediatePoints.push([lng2, lat2]);
                }
                nextSegment = {
                  ...foundRoad,
            ...
            if (!foundRoad) {
              ...
              const straightAheadDistance = stepDistance * maxSteps;
              const radians = currentBearing * Math.PI / 180;
              const intermediateSteps = 10;
              const syntheticCoords = [];
              for (let i = 1; i <= intermediateSteps; i++) {
                const progress = i / intermediateSteps;
                const lng2 = lastPoint[0] + straightAheadDistance * progress * Math.sin(radians);
                const lat2 = lastPoint[1] + straightAheadDistance * progress * Math.cos(radians);
                syntheticCoords.push([lng2, lat2]);
              }
              nextSegment = {
          ...
          if (nextSegment) {
            if (nextSegment.coords.length > 2) {
              currentSegmentCoords = vehicleProfile.smoothPath ? resamplePathCatmullRom(nextSegment.coords.slice(1), 0.01) : resamplePath(nextSegment.coords.slice(1), 0.01);
            } else {
              currentSegmentCoords = vehicleProfile.smoothPath ? resamplePathCatmullRom(nextSegment.coords, 0.01) : resamplePath(nextSegment.coords, 0.01);
              console.log(`[RoadChain] Short segment - keeping all ${nextSegment.coords.length} points for bearing calc`);
            }
            currentSegmentCoords.roadClass = (_g = nextSegment.road.properties) == null ? void 0 : _g.class;
            currentSegmentCoords.roadName = (_h = nextSegment.road.properties) == null ? void 0 : _h.name;
            currentSegmentCoords.roadRef = (_i = nextSegment.road.properties) == null ? void 0 : _i.ref;
            currentSegmentIndex = 0;
            segmentCount++;
            usedSegmentIds.add(nextSegment.road.id);
            const segmentClass = ((_j = nextSegment.road.properties) == null ? void 0 : _j.class) || "road";
            const segmentName = (_k = nextSegment.road.properties) == null ? void 0 : _k.name;
            const segmentRef = (_l = nextSegment.road.properties) == null ? void 0 : _l.ref;
            const roadIdentity = segmentRef || segmentName || segmentClass;
            const distanceM = nextSegment.distance ? (nextSegment.distance * 1e3).toFixed(1) : "0.0";
            console.log(`[RoadChain] ✅ Segment #${segmentCount}: ${roadIdentity} (${nextSegment.coords.length} pts, ${nextSegment.reversed ? "reversed" : "forward"}, bearing Δ${nextSegment.bearingDiff.toFixed(1)}°, ${distanceM}m)` + (nextSegment.score ? `, score: ${nextSegment.score.toFixed(1)}` : ""));
        ...
        if (currentSegmentIndex < currentSegmentCoords.length - 1) {
          ...
          moveDuration = distanceKm / vehicleSpeedKmh * 3600 * 1e3;
        ...
        if (elevation !== null && elevation >= 0) {
          const elevationKm = elevation / 1e3;
        ...
        map.easeTo({
          ...
          // Linear for smooth continuous motion
          noMoveStart: true,
          // Don't trigger movestart event for smoother transitions
          delayEndEvents: 0
          // Don't delay end events
        });
        await map.once("moveend");
        currentSegmentIndex++;
        totalPointsVisited++;
  ...
  tractorRoadTrip: (map, control, options = {}) => {
    const profile = {
      altitude: 8,
      zoom: 20,
      // Very close for slow speed
      pitch: 60,
      smoothing: 5,
      speedKmh: 30,
      // Slow tractor speed
      searchRadius: 2e-3,
      // 200m search radius for ground vehicle
      preloadDistance: 2e-3,
      // 200m preload for slow vehicle
      icon: "🚜",
      name: "Tractor Road Trip",
      supportsExploration: true,
      // Road-aware animation
      smoothPath: true
  ...
  carRoadTrip: (map, control, options = {}) => {
    const profile = {
      ...
      searchRadius: 2e-3,
      // 200m search radius for ground vehicle
      preloadDistance: 5e-3,
      // 500m preload for car speed
      icon: "🚗",
      name: "Car Road Trip",
      supportsExploration: true,
      // Road-aware animation
  ...
  sportsCarRace: (map, control, options = {}) => {
    const profile = {
      altitude: 25,
      zoom: 17.5,
      // Higher up to see further ahead at high speed
      pitch: 60,
      smoothing: 5,
      speedKmh: 130,
      // Sports car racing speed
      searchRadius: 3e-3,
      // 300m search radius for fast vehicle
      preloadDistance: 0.01,
      // 1km preload for high speed
      icon: "🏎️",
  ...
  planeFlight: (map, control, options = {}) => {
    const profile = {
      ...
      smoothing: 8,
      speedKmh: 200,
      // Plane cruising speed
      searchRadius: 0.01,
      // 1km search radius for high altitude
      preloadDistance: 0.015,
      // 1.5km preload for plane speed
      icon: "✈️",
      name: "Plane Flight",
      supportsExploration: true,
      // Road-aware animation
  ...
  helicopterTour: (map, control, options = {}) => {
    const profile = {
      ...
      smoothing: 6,
      speedKmh: 60,
      // Helicopter touring speed
      searchRadius: 5e-3,
      // 500m search radius for medium altitude
      preloadDistance: 5e-3,
      // 500m preload for helicopter
      icon: "🚁",
  ...
  droneFollow: (map, control, options = {}) => {
    const profile = {
      ...
      smoothing: 4,
      speedKmh: 60,
      // Drone filming speed (increased for better pacing)
      searchRadius: 5e-3,
      // 500m search radius for drone (larger than ground vehicles)
      preloadDistance: 4e-3,
      // 400m preload for drone
      icon: "🛸",
      name: "Drone Follow",
      supportsExploration: true,
      // Road-aware animation
  ...
  birdsEyeRoad: (map, control, options = {}) => {
    const profile = {
      ...
      // Bird flight speed
      searchRadius: 0.01,
      // 1km search radius for high altitude flight
      preloadDistance: 4e-3,
      // 400m preload for bird
      icon: "🦅",
  ...
  trainRide: (map, control, options = {}) => {
    const profile = {
      ...
      pitch: 55,
      smoothing: 8,
      // Trains are very smooth and stable
      speedKmh: 70,
      // Moderate train speed
      searchRadius: 2e-3,
      // 200m search radius for ground transport
      preloadDistance: 5e-3,
      // 500m preload for train
      icon: "🚂",
      name: "Train Ride",
      supportsExploration: true,
      // Path-aware animation
      transportClasses: ["rail", "transit"],
      // Follow railway tracks instead of roads
      smoothPath: true
      // Smooth Catmull-Rom curves
    };
    return PresetAnimations._followPathWithVehicleSetup(map, control, options, profile);
  ...
  speedboat: (map, control, options = {}) => {
    const profile = {
      ...
      // Fast speedboat
      searchRadius: 5e-3,
      // 500m search radius for fast watercraft
      preloadDistance: 7e-3,
      // 700m preload for speedboat
      icon: "🚤",
      name: "Speedboat",
      supportsExploration: true,
      // Path-aware animation
      transportClasses: ["river", "canal", "stream"],
      // Follow all waterways
      smoothPath: true
      // Smooth Catmull-Rom curves
    };
    return PresetAnimations._followPathWithVehicleSetup(map, control, options, profile);
  ...
  sailboat: (map, control, options = {}) => {
    const profile = {
      ...
      pitch: 55,
      smoothing: 7,
      // Stable but not too rigid
      speedKmh: 28,
      // Moderate sailing speed
      searchRadius: 4e-3,
      // 400m search radius for waterways
      preloadDistance: 2e-3,
      // 200m preload for sailboat
      icon: "⛵",
      name: "Sailboat",
      supportsExploration: true,
      // Path-aware animation
      transportClasses: ["river", "canal"],
      // Follow rivers and canals (not small streams)
      smoothPath: true
      // Smooth Catmull-Rom curves
    };
    return PresetAnimations._followPathWithVehicleSetup(map, control, options, profile);
  ...
  cruiseShip: (map, control, options = {}) => {
    const profile = {
      ...
      // 400m search radius for waterways
      preloadDistance: 2e-3,
      // 200m preload for cruise ship
      icon: "🛥️",
      name: "Cruise Ship",
      supportsExploration: true,
      // Path-aware animation
      transportClasses: ["river", "canal"],
      // Follow major waterways only
      smoothPath: true
      // Smooth Catmull-Rom curves
    };
    return PresetAnimations._followPathWithVehicleSetup(map, control, options, profile);
  ...
  freeFlight: async (map, { updateStatus, checkAbort }, options = {}) => {
    const duration = options.duration || 6e4;
    const speedKmh = options.speedKmh || 80;
    const pitch = options.pitch || 50;
    updateStatus("✈️ Free flight - cruising forward...");
    const startTime = maplibregl.now();
    const initialBearing = map.getBearing();
    const initialCenter = map.getCenter();
    map.easeTo({ pitch, duration: 2e3, essential: true });
    await map.once("moveend");
    checkAbort();
    const speedMs = speedKmh * 1e3 / 3600;
    const stepInterval = 100;
    const distancePerStep = speedMs * (stepInterval / 1e3);
    const degreesPerStep = distancePerStep / 111e3;
    ...
    while (true) {
      ...
      if (!maplibregl.isTimeFrozen || !maplibregl.isTimeFrozen()) {
        if (elapsed >= duration) {
          console.log(`[FreeFlight] Cruise complete: ${(elapsed / 1e3).toFixed(1)}s`);
          break;
        }
      }
      const t = elapsed / 1e3;
      bearingDrift += Math.sin(t * 0.1) * 0.02 + (Math.random() - 0.5) * 0.05;
      bearingDrift = Math.max(-5, Math.min(5, bearingDrift));
...
// node_modules/maplibre-gl-video-export/src/controller.js
var AnimationController = class {
  ...
  get aborted() {
    var _a;
    return ((_a = this.abortController) == null ? void 0 : _a.signal.aborted) ?? false;
  }
  /**
...
// node_modules/maplibre-gl-video-export/src/webm-encoder-wrapper.js
var WebmEncoderWrapper = class {
  constructor() {
    this.worker = null;
...
Error: ${error.message}`
    ...
    this.worker.onmessage = (e) => {
      ...
      } else if (e.data === null || e.data === void 0) {
    ...
    await new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        this.resolveReady = null;
        reject(new Error(
          "WebM worker initialization timeout. Make sure vendor/webm/ files are correctly deployed."
        ));
      }, 3e4);
      this.resolveReady = () => {
    ...
    const config = {
      ...
      // bitrate in kbps
      realtime
      // Realtime mode: false = high quality (slower), true = fast encoding
    };
...
// node_modules/maplibre-gl-video-export/src/index.js
var asInput = (target) => (
...
var asButton = (element) => (
  /** @type {HTMLButtonElement | null} */
  element
);
var MP4_ENCODER_CDN = "https://unpkg.com/mp4-h264@1.0.7/build/";
var WASM_FEATURE_DETECT_URL = "https://unpkg.com/wasm-feature-detect?module";
...
var ANIMATION_PROFILES = {
  ...
  smart: {
    ...
    group: "auto",
    requires: [],
    // Smart adapts to whatever is available
  ...
  neighborhood: {
    ...
    supportsExploration: false,
    group: "cinematic",
    requires: ["?hasPlaces", "?hasRoads"],
    // Better with places and roads
    /** @type {AnimationFunction} */
  ...
  property: {
    ...
    supportsExploration: false,
    group: "cinematic",
    requires: ["?has3DBuildings"],
    // Better with 3D buildings
    /** @type {AnimationFunction} */
  ...
  terrainFollowing: {
    ...
    supportsExploration: false,
    group: "cinematic",
    requires: ["?hasTerrain", "?hasHillshade"],
    // Better with terrain
    /** @type {AnimationFunction} */
  ...
  waypointTour: {
    ...
    group: "waypoint",
    requires: [],
    // Works with user-provided waypoints
    /** @type {AnimationFunction} */
    func: (_map, _control, options) => ({ animation: async (m, callbacks) => PresetAnimations.waypointTour(m, callbacks, options) })
  }
};
...
var _VideoExportControl = class _VideoExportControl {
  /**
  ...
  constructor(options = {}) {
    this.options = {
      ...
      // 'auto', 'fullhd', 'hd', '4k', '8k', or {width, height}
      fps: options.fps || 60,
      bitrate: options.bitrate !== void 0 ? options.bitrate : "auto",
      // 'auto' or kbps value
      speedMultiplier: options.speedMultiplier || 1,
      // Animation speed multiplier (1 = real-time)
      waitForTiles: options.waitForTiles !== void 0 ? options.waitForTiles : true,
      // Wait for tiles to load before each frame
      // UI settings
      position: options.position || "top-left",
      collapsed: options.collapsed !== false,
      compactPosition: options.compactPosition || "top-left",
      // Position when in compact mode: 'top-left', 'top-right', 'bottom-left', 'bottom-right'
      // Animation
      animation: options.animation || "smart",
      // 'smart', 'orbit', 'pulse', or function
      duration: options.duration || 3e4,
      // Total animation duration in ms
      loop: options.loop || false,
      // false, true/'instant', or 'smooth'
      // Exploration limits
      explorationLimitEnabled: options.explorationLimitEnabled !== void 0 ? options.explorationLimitEnabled : false,
      // Enable/disable exploration duration limit
      explorationMaxDuration: options.explorationMaxDuration || 3e5,
      // Maximum exploration duration in ms (default: 5 minutes)
      // Geographic constraints
      maxBounds: options.maxBounds || null,
      // [[west, south], [east, north]] or LngLatBounds
      minZoom: options.minZoom !== void 0 ? options.minZoom : null,
      // Minimum zoom level (null = no limit)
      maxZoom: options.maxZoom !== void 0 ? options.maxZoom : null,
      // Maximum zoom level (null = no limit)
      strictBounds: options.strictBounds || false,
      // If true, strictly enforce bounds (no partial view outside)
      showBoundsOverlay: options.showBoundsOverlay !== void 0 ? options.showBoundsOverlay : false,
      // Show visual bounds on map
      // Waypoints (Points of Interest)
      waypoints: options.waypoints || null,
      // Array of waypoint objects [{center: [lng, lat], zoom, duration, bearing, pitch, name, icon}]
      // Video format - auto-detects VP9 support and uses it by default for better quality
      format: options.format || getDefaultFormat(),
      // 'webm-vp8', 'webm-vp9' (default if supported), or 'mp4'
      // Encoder paths - auto-detects plugin location first, then CDN fallback for MP4
      encoderPath: options.encoderPath || null,
      // Custom path (optional)
      encoderCdn: options.encoderCdn || MP4_ENCODER_CDN,
      // MP4 CDN fallback
      // Callbacks
    ...
    this._savedWaypointsVisibility = void 0;
    this._waypointsLayerId = "ve-waypoints-recording-layer";
    this._waypointsSourceId = "ve-waypoints-recording-source";
    ...
    this._spritePngUrl = null;
    this._iconSize = 1;
    this._availableFonts = [];
    this._selectedFont = null;
    this._showWaypointLabels = false;
  ...
  onRemove() {
    ...
    const mapContainer = (_a = this._map) == null ? void 0 : _a.getContainer();
    if (mapContainer) {
  ...
  _checkMapLibreVersion() {
    const hasTimeControl = typeof maplibregl !== "undefined" && // @ts-ignore - timeControl API properties may not exist in older versions
    typeof maplibregl.setNow === "function" && typeof maplibregl.restoreNow === "function" && typeof maplibregl.isTimeFrozen === "function";
    if (!hasTimeControl) {
      const version = this._map && this._map.version ? this._map.version : "unknown";
      throw new Error(
        "MapLibre GL JS v5.10.0 or higher is required for video recording. The timeControl API (setNow, restoreNow, isTimeFrozen) is missing. Please upgrade to MapLibre GL JS v5.10.0+. Current version: " + version
      );
    }
    ...
    if (!hasNow) {
      const version = this._map && this._map.version ? this._map.version : "unknown";
      throw new Error(
        "MapLibre GL JS with exported now() function is required for vehicle animations. The official v5.10.0 does not export now() - see https://github.com/maplibre/maplibre-gl-js/issues/6643. Please use a custom build of MapLibre that exports now() from time_control.ts. Current version: " + version
      );
    }
  ...
  _loadSettings() {
    ...
    return saved ? JSON.parse(saved) : _VideoExportControl.DEFAULT_SETTINGS;
  }
  /**
    ...
    this._panel.querySelectorAll('input[id^="ve-"], select[id^="ve-"], textarea[id^="ve-"]').forEach((el) => {
      if (el.type === "checkbox") {
        ...
        if (el.id.endsWith("-custom")) {
          ...
          parentId = parentId.replace(/-(width|height)$/, "");
          const selectEl = document.getElementById(parentId);
          if ((selectEl == null ? void 0 : selectEl.value) === "custom") {
  ...
  _saveWaypoints() {
    ...
    try {
      ...
      console.log(`[Waypoints] Saved ${((_a = this.options.waypoints.features) == null ? void 0 : _a.length) || 0} waypoints to localStorage`);
    } catch (e) {
  ...
  _loadSectionStates() {
    ...
    const defaults = {
      "video-settings": false,
      // Open by default
      movie: false,
      // Open by default
      "points-of-interest": true,
      // Collapsed by default
      "geographic-constraints": true
      // Collapsed by default
    };
  ...
  _toggleSection(sectionId) {
    ...
    const header = (_a = this._panel) == null ? void 0 : _a.querySelector(`[data-section="${sectionId}"]`);
    const content = (_b = this._panel) == null ? void 0 : _b.querySelector(`[data-section-content="${sectionId}"]`);
    const indicator = header == null ? void 0 : header.querySelector(".section-indicator");
  ...
  _bindEvents() {
    ...
    (_a = this._panel.querySelector("#ve-test")) == null ? void 0 : _a.addEventListener("click", () => this._testAnimation());
    (_b = this._panel.querySelector("#ve-explore")) == null ? void 0 : _b.addEventListener("click", () => this._startExploration());
    (_c = this._panel.querySelector("#ve-record")) == null ? void 0 : _c.addEventListener("click", () => this._startRecording());
    (_d = this._panel.querySelector("#ve-reset-defaults")) == null ? void 0 : _d.addEventListener("click", (e) => {
      e.preventDefault();
      if (!confirm("Reset all settings to default values?")) return;
      this._applySettings(_VideoExportControl.DEFAULT_SETTINGS);
      const resetMessage = this._panel.querySelector("#ve-reset-message");
      if (resetMessage) resetMessage.style.display = "block";
    });
    (_e = this._panel.querySelector("#ve-cancel-reset")) == null ? void 0 : _e.addEventListener("click", (e) => {
      e.preventDefault();
    ...
    const updateRecordingTime = () => {
      ...
      const seconds = Math.round(recordingDuration / 1e3);
      const timeDisplay = this._panel.querySelector("#ve-recording-time");
      if (timeDisplay) {
        timeDisplay.textContent = `${seconds}s`;
      }
    };
    (_f = this._panel.querySelector("#ve-animation")) == null ? void 0 : _f.addEventListener("change", (e) => {
      var _a2;
      this.options.animation = ((_a2 = asSelect(e.target)) == null ? void 0 : _a2.value) || "orbit";
      this._updateAnimationDescription();
    ...
    resolutionSelect == null ? void 0 : resolutionSelect.addEventListener("change", (e) => {
      var _a2, _b2;
      if (((_a2 = asSelect(e.target)) == null ? void 0 : _a2.value) === "custom") {
        if (resolutionCustomGroup) resolutionCustomGroup.style.display = "block";
        this.options.resolution = {
          width: parseInt((resolutionWidthInput == null ? void 0 : resolutionWidthInput.value) || "1920", 10),
          height: parseInt((resolutionHeightInput == null ? void 0 : resolutionHeightInput.value) || "1080", 10)
        };
      } else {
        if (resolutionCustomGroup) resolutionCustomGroup.style.display = "none";
        this.options.resolution = ((_b2 = asSelect(e.target)) == null ? void 0 : _b2.value) || "1920x1080";
      }
    });
    resolutionWidthInput == null ? void 0 : resolutionWidthInput.addEventListener("input", (e) => {
      var _a2;
      if ((resolutionSelect == null ? void 0 : resolutionSelect.value) === "custom") {
        this.options.resolution = {
          width: parseInt(((_a2 = asInput(e.target)) == null ? void 0 : _a2.value) || "1920", 10),
          height: parseInt((resolutionHeightInput == null ? void 0 : resolutionHeightInput.value) || "1080", 10)
        };
      }
    });
    resolutionHeightInput == null ? void 0 : resolutionHeightInput.addEventListener("input", (e) => {
      var _a2;
      if ((resolutionSelect == null ? void 0 : resolutionSelect.value) === "custom") {
        this.options.resolution = {
          width: parseInt((resolutionWidthInput == null ? void 0 : resolutionWidthInput.value) || "1920", 10),
          height: parseInt(((_a2 = asInput(e.target)) == null ? void 0 : _a2.value) || "1080", 10)
        };
      }
    });
    ...
    if (durationSelect) {
      if (durationSelect.value === "custom") {
        this.options.duration = parseFloat((durationInput == null ? void 0 : durationInput.value) || "30") * 1e3;
      } else {
        this.options.duration = parseFloat(durationSelect.value || "30") * 1e3;
      }
    }
    const updateRealTimeDisplay = () => {
      const virtualTime = this.options.duration / 1e3;
    ...
    if (durationSelect) {
      durationSelect.addEventListener("change", (e) => {
        var _a2;
        const value = (_a2 = asSelect(e.target)) == null ? void 0 : _a2.value;
        if (value === "custom") {
          ...
          this.options.duration = parseFloat(value || "30") * 1e3;
        }
    ...
    if (durationInput) {
      durationInput.addEventListener("input", (e) => {
        var _a2;
        this.options.duration = parseFloat(((_a2 = asInput(e.target)) == null ? void 0 : _a2.value) || "30") * 1e3;
        updateRealTimeDisplay();
      });
    }
    (_g = this._panel.querySelector("#ve-fps")) == null ? void 0 : _g.addEventListener("input", (e) => {
      var _a2;
      this.options.fps = parseFloat(((_a2 = asInput(e.target)) == null ? void 0 : _a2.value) || "30");
    });
    (_h = this._panel.querySelector("#ve-wait-tiles")) == null ? void 0 : _h.addEventListener("change", (e) => {
      var _a2;
      this.options.waitForTiles = ((_a2 = asInput(e.target)) == null ? void 0 : _a2.checked) ?? true;
    });
    (_i = this._panel.querySelector("#ve-loop")) == null ? void 0 : _i.addEventListener("change", (e) => {
      var _a2;
      const value = (_a2 = asSelect(e.target)) == null ? void 0 : _a2.value;
      if (value === "false") {
    ...
    (_j = this._panel.querySelector("#ve-format")) == null ? void 0 : _j.addEventListener("change", (e) => {
      var _a2;
      if (!this._panel) return;
      this.options.format = ((_a2 = asSelect(e.target)) == null ? void 0 : _a2.value) || "webm-vp8";
    ...
    const vp9Option = (
      ...
      this._panel.querySelector("#ve-format-vp9")
    );
    ...
    formatSelect == null ? void 0 : formatSelect.dispatchEvent(new Event("change"));
    setTimeout(() => {
      updateRealTimeDisplay();
    }, 0);
    ...
    formatAdvancedToggle == null ? void 0 : formatAdvancedToggle.addEventListener("change", (e) => {
      var _a2, _b2;
      if (!this._panel) return;
      if (formatAdvancedGroup) formatAdvancedGroup.style.display = ((_a2 = asInput(e.target)) == null ? void 0 : _a2.checked) ? "block" : "none";
      const mp4Advanced = asHTMLElement(this._panel.querySelector("#ve-mp4-advanced"));
      const vp8Advanced = asHTMLElement(this._panel.querySelector("#ve-webm-vp8-advanced"));
      const vp9Advanced = asHTMLElement(this._panel.querySelector("#ve-webm-vp9-advanced"));
      if (((_b2 = asInput(e.target)) == null ? void 0 : _b2.checked) && mp4Advanced && vp8Advanced && vp9Advanced) {
        mp4Advanced.style.display = "none";
        vp8Advanced.style.display = "none";
        vp9Advanced.style.display = "none";
        if (this.options.format === "mp4") {
    ...
    if (speedSelect) {
      if (speedSelect.value === "custom") {
        this.options.speedMultiplier = parseFloat((speedCustomInput == null ? void 0 : speedCustomInput.value) || "1");
      } else {
        this.options.speedMultiplier = parseFloat(speedSelect.value || "1");
      }
    }
    speedSelect == null ? void 0 : speedSelect.addEventListener("change", (e) => {
      var _a2, _b2;
      if (((_a2 = asSelect(e.target)) == null ? void 0 : _a2.value) === "custom") {
        ...
        this.options.speedMultiplier = parseFloat(((_b2 = asSelect(e.target)) == null ? void 0 : _b2.value) || "1");
      }
      updateRealTimeDisplay();
    });
    speedCustomInput == null ? void 0 : speedCustomInput.addEventListener("input", (e) => {
      var _a2;
      this.options.speedMultiplier = parseFloat(((_a2 = asInput(e.target)) == null ? void 0 : _a2.value) || "1");
      updateRealTimeDisplay();
    });
    ...
    bitrateSelect == null ? void 0 : bitrateSelect.addEventListener("change", (e) => {
      var _a2, _b2, _c2;
      if (((_a2 = asSelect(e.target)) == null ? void 0 : _a2.value) === "custom") {
        if (bitrateCustomGroup) bitrateCustomGroup.style.display = "block";
        this.options.bitrate = parseInt((bitrateCustomInput == null ? void 0 : bitrateCustomInput.value) || "5000", 10);
      } else if (((_b2 = asSelect(e.target)) == null ? void 0 : _b2.value) === "auto") {
        if (bitrateCustomGroup) bitrateCustomGroup.style.display = "none";
        this.options.bitrate = "auto";
      } else {
        if (bitrateCustomGroup) bitrateCustomGroup.style.display = "none";
        this.options.bitrate = parseInt(((_c2 = asSelect(e.target)) == null ? void 0 : _c2.value) || "5000", 10);
      }
    });
    bitrateCustomInput == null ? void 0 : bitrateCustomInput.addEventListener("input", (e) => {
      var _a2;
      this.options.bitrate = parseInt(((_a2 = asInput(e.target)) == null ? void 0 : _a2.value) || "5000", 10);
    });
    ...
    if (boundsWaypointsBtn) {
      boundsWaypointsBtn.addEventListener("click", () => {
        var _a2;
        if (!this._panel) return;
        const features = ((_a2 = this.options.waypoints) == null ? void 0 : _a2.features) || [];
    ...
    boundInputs.forEach((selector) => {
      var _a2;
      const input = (_a2 = this._panel) == null ? void 0 : _a2.querySelector(selector);
      if (input) {
    ...
    if (strictBoundsCheck) {
      strictBoundsCheck.addEventListener("change", (e) => {
        var _a2;
        this.options.strictBounds = ((_a2 = asInput(e.target)) == null ? void 0 : _a2.checked) ?? false;
      });
    }
    ...
    if (showBoundsCheck) {
      showBoundsCheck.addEventListener("change", (e) => {
        var _a2;
        const checked = ((_a2 = asInput(e.target)) == null ? void 0 : _a2.checked) ?? false;
    ...
    if (fontSelect) {
      fontSelect.addEventListener("change", (e) => {
        var _a2;
        this._selectedFont = ((_a2 = asSelect(e.target)) == null ? void 0 : _a2.value) || "Roboto";
    ...
    if (iconSizeSlider && iconSizeValue) {
      iconSizeSlider.addEventListener("input", (e) => {
        var _a2;
        this._iconSize = parseFloat(((_a2 = asInput(e.target)) == null ? void 0 : _a2.value) || "1");
  ...
  _updateBoundsFromUI() {
    var _a, _b, _c, _d;
    if (!this._panel) return;
    const west = parseFloat(((_a = asInput(this._panel.querySelector("#ve-bounds-west"))) == null ? void 0 : _a.value) || "");
    const east = parseFloat(((_b = asInput(this._panel.querySelector("#ve-bounds-east"))) == null ? void 0 : _b.value) || "");
    const south = parseFloat(((_c = asInput(this._panel.querySelector("#ve-bounds-south"))) == null ? void 0 : _c.value) || "");
    const north = parseFloat(((_d = asInput(this._panel.querySelector("#ve-bounds-north"))) == null ? void 0 : _d.value) || "");
  ...
  _updateZoomLimitsFromUI() {
    var _a, _b;
    if (!this._panel) return;
    const minZoom = parseFloat(((_a = asInput(this._panel.querySelector("#ve-zoom-min"))) == null ? void 0 : _a.value) || "");
    const maxZoom = parseFloat(((_b = asInput(this._panel.querySelector("#ve-zoom-max"))) == null ? void 0 : _b.value) || "");
  ...
  async _loadFontstacks() {
    try {
      ...
      try {
        ...
        if (response.ok) {
          const fontstacks2 = await response.json();
          if (Array.isArray(fontstacks2) && fontstacks2.length > 0) {
            this._availableFonts = fontstacks2;
            console.log(`[Waypoints] ✓ Loaded ${fontstacks2.length} font stacks from fontstacks.json`);
            if (!this._selectedFont) {
              this._selectedFont = fontstacks2[0];
  ...
  _fillIconSelect(filter = "") {
    ...
    if (this._spriteIcons.length > 0) {
      ...
      const iconsToShow = filter ? (this._allIcons || []).filter((id) => id.toLowerCase().includes(filter.toLowerCase())) : this._allIcons || [];
  ...
  _updateIconPreview() {
    ...
    if (this._spriteData && this._spriteData[selectedIcon] && this._spritePngUrl && this._spriteImage && this._spriteImage.complete) {
  ...
  _fillPopupIconSelect(index, filter = "") {
    ...
    if (!iconSelect) return;
    const currentValue = (_a = asSelect(iconSelect)) == null ? void 0 : _a.value;
    iconSelect.innerHTML = "";
    if (this._spriteIcons.length > 0) {
      if (!filter || "waypoint-default".includes(filter.toLowerCase()) || "default".includes(filter.toLowerCase())) {
        const defaultOption = document.createElement("option");
      ...
      const iconsToShow = filter ? this._spriteIcons.filter((id) => id.toLowerCase().includes(filter.toLowerCase())) : this._spriteIcons;
      iconsToShow.forEach((iconId) => {
        const option = document.createElement("option");
        option.value = iconId;
  ...
  _updatePopupIconPreview(index) {
    ...
    if (!iconSelect || !previewDiv) return;
    const selectedIcon = (_a = asSelect(iconSelect)) == null ? void 0 : _a.value;
    previewDiv.innerHTML = "";
    if (!selectedIcon) {
      const span2 = document.createElement("span");
      span2.style.fontSize = "20px";
    ...
    if (this._spriteData && this._spriteData[selectedIcon] && this._spritePngUrl && this._spriteImage && this._spriteImage.complete && this._spriteImage.naturalWidth > 0 && this._spriteImage.naturalHeight > 0) {
      const iconData = this._spriteData[selectedIcon];
  ...
  _createMarkerElement(iconId, index) {
    if (!this._spriteData || !this._spriteData[iconId] || !this._spritePngUrl) {
      ...
      el2.className = "ve-waypoint-marker";
      el2.innerHTML = `<svg width="24" height="36" viewBox="0 0 24 36" xmlns="http://www.w3.org/2000/svg">
                <ellipse cx="12" cy="34" rx="4" ry="2" fill="rgba(0,0,0,0.3)" />
                <path d="M12 2 C7 2 3 6 3 11 C3 16 12 26 12 26 C12 26 21 16 21 11 C21 6 17 2 12 2 Z" fill="white" />
                <path d="M12 4 C8 4 5 7 5 11 C5 15 12 24 12 24 C12 24 19 15 19 11 C19 7 16 4 12 4 Z" fill="#3887be" />
                <circle cx="12" cy="11" r="3" fill="white" opacity="0.9" />
      ...
      el2.style.cssText = `
            ...
            `;
      el2.dataset.waypointIndex = String(index);
      return el2;
    }
    ...
    return `
            <div class="ve-waypoint-popup" style="min-width: 280px; max-width: 320px; max-height: 70vh; overflow-y: auto;">
                ...
                <div style="margin-bottom: 6px;">
                    ...
                    <input type="number" value="${props.duration || 2e3}" step="100" placeholder="e.g., 3000"
  ...
  _createWaypointMarkers() {
    ...
    features.forEach((feature, index) => {
      ...
      if (iconName === "waypoint-default") {
        ...
        if (this._spriteIcons.includes(iconName)) {
          ...
          iconId = this._spriteIcons.find((icon) => {
            ...
            return iconLower.includes(searchTerm) || iconLower.startsWith(searchTerm + "-") || iconLower.startsWith(searchTerm + "_");
          });
        }
        if (!iconId) {
          iconId = "waypoint-default";
      ...
      const marker = new maplibregl.Marker({
        ...
        draggable: true,
        anchor: "bottom"
        // Anchor at bottom center (like a pin)
  ...
  _attachPopupEventListeners(index, popup) {
    ...
    inputs.forEach((input) => {
      input.addEventListener("change", (e) => {
        ...
        const field = (
          /** @type {HTMLElement} */
          (_a = e.target) == null ? void 0 : _a.dataset.field
        );
        if (!field) return;
        let value = (_b = asInput(e.target)) == null ? void 0 : _b.value;
    ...
    if (iconSelect) {
      iconSelect.addEventListener("change", (e) => {
        var _a;
        const iconValue = (_a = asSelect(e.target)) == null ? void 0 : _a.value;
        if (!iconValue) return;
    ...
    if (iconSearch && iconSelect) {
      iconSearch.addEventListener("input", (e) => {
        var _a;
        const filter = ((_a = asInput(e.target)) == null ? void 0 : _a.value) || "";
    ...
    if (lngField && latField) {
      lngField.addEventListener("change", (e) => {
        var _a;
        const value = parseFloat(((_a = asInput(e.target)) == null ? void 0 : _a.value) || "");
        if (!isNaN(value)) {
    ...
    if (cameraToggle && zoomField && bearingField && pitchField) {
      ...
      if (feature.properties.zoom !== void 0 || feature.properties.bearing !== void 0 || feature.properties.pitch !== void 0) {
        if (cameraToggleEl) cameraToggleEl.checked = true;
        if (zoomFieldEl) {
          zoomFieldEl.disabled = false;
          if (feature.properties.zoom !== void 0) zoomFieldEl.value = feature.properties.zoom.toString();
        }
        if (bearingFieldEl) {
          bearingFieldEl.disabled = false;
          if (feature.properties.bearing !== void 0) bearingFieldEl.value = feature.properties.bearing.toString();
        }
        if (pitchFieldEl) {
          pitchFieldEl.disabled = false;
          if (feature.properties.pitch !== void 0) pitchFieldEl.value = feature.properties.pitch.toString();
        }
        if (this._map) {
      ...
      cameraToggle.addEventListener("change", (e) => {
        var _a;
        const checked = (_a = asInput(e.target)) == null ? void 0 : _a.checked;
        if (checked) {
      ...
      [zoomField, bearingField, pitchField].forEach((field) => {
        field.addEventListener("change", (e) => {
          ...
          const fieldId = (inputEl == null ? void 0 : inputEl.id) || "";
          if (value && value !== "") {
  ...
  async _createWaypointsWebGLLayer() {
    ...
    this._map.addLayer({
      ...
      layout: {
        ...
        // Use the resolved iconId
        "icon-size": this._iconSize || 1,
        "icon-allow-overlap": true,
        "icon-ignore-placement": true,
        // Force rendering even if overlaps
        "icon-anchor": "bottom",
        // Match marker anchor
        visibility: "visible",
...
Add anyway?`
  ...
  _updateWaypointsUI() {
    ...
    features.forEach((feature, index) => {
      ...
      let iconHTML = '<span style="font-size: 18px;">❓</span>';
      ...
      nameSpan.textContent = props.name || `Waypoint ${index + 1}`;
    ...
    list.querySelectorAll(".ve-wp-move-up").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        const index = parseInt(btn.getAttribute("data-index") || "0", 10);
        if (index > 0) {
          const features2 = this.options.waypoints.features;
          [features2[index - 1], features2[index]] = [features2[index], features2[index - 1]];
          this._updateWaypointsUI();
          this._createWaypointMarkers();
          this._saveWaypoints();
    ...
    list.querySelectorAll(".ve-wp-move-down").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        const index = parseInt(btn.getAttribute("data-index") || "0", 10);
        const features2 = this.options.waypoints.features;
        if (index < features2.length - 1) {
          [features2[index], features2[index + 1]] = [features2[index + 1], features2[index]];
          this._updateWaypointsUI();
          this._createWaypointMarkers();
          this._saveWaypoints();
          console.log(`[Waypoints] Moved waypoint from ${index} to ${index + 1}`);
  ...
  _editWaypoint(index) {
    ...
    const hasCamera = props.zoom !== void 0 || props.bearing !== void 0 || props.pitch !== void 0;
    ...
    if (hasCamera) {
      ...
      if (zoomInput) zoomInput.value = props.zoom !== void 0 ? props.zoom : "";
      if (bearingInput) bearingInput.value = props.bearing !== void 0 ? props.bearing : "";
      if (pitchInput) pitchInput.value = props.pitch !== void 0 ? props.pitch : "";
    } else {
  ...
  _saveWaypoint() {
    ...
    if (!this._panel) return;
    const editor = this._panel.querySelector("#ve-waypoint-editor");
    const index = parseInt(((_a = asInput(this._panel.querySelector("#ve-wp-index"))) == null ? void 0 : _a.value) || "0", 10);
    const features = this.options.waypoints.features;
    if (!features || !features[index]) return;
    const icon = ((_b = asSelect(this._panel.querySelector("#ve-wp-icon"))) == null ? void 0 : _b.value) || "";
    const name = (_c = asInput(this._panel.querySelector("#ve-wp-name"))) == null ? void 0 : _c.value;
    const lng = parseFloat(((_d = asInput(this._panel.querySelector("#ve-wp-lng"))) == null ? void 0 : _d.value) || "0");
    const lat = parseFloat(((_e = asInput(this._panel.querySelector("#ve-wp-lat"))) == null ? void 0 : _e.value) || "0");
    const zoom = (_f = asInput(this._panel.querySelector("#ve-wp-zoom"))) == null ? void 0 : _f.value;
    const bearing = (_g = asInput(this._panel.querySelector("#ve-wp-bearing"))) == null ? void 0 : _g.value;
    const pitch = (_h = asInput(this._panel.querySelector("#ve-wp-pitch"))) == null ? void 0 : _h.value;
    const duration = (_i = asInput(this._panel.querySelector("#ve-wp-duration"))) == null ? void 0 : _i.value;
...
Save anyway?`
  ...
  _deleteWaypoint(index) {
    ...
    if (confirm(`Delete waypoint "${name}"?`)) {
      ...
      this._saveWaypoints();
      const editor = asHTMLElement(this._panel.querySelector("#ve-waypoint-editor"));
      const editingIndex = parseInt(((_a = asInput(this._panel.querySelector("#ve-wp-index"))) == null ? void 0 : _a.value) || "-1", 10);
  ...
  _importWaypoints() {
    ...
    input.onchange = (e) => {
      var _a, _b;
      const file = (_b = (_a = asInput(e.target)) == null ? void 0 : _a.files) == null ? void 0 : _b[0];
      if (!file) return;
...
Please upload a .json or .geojson file.`
      ...
      reader.onerror = () => {
        ...
        alert(
          `Error reading file: ${((_a2 = reader.error) == null ? void 0 : _a2.message) || "Unknown error"}
      ...
      reader.onload = (event) => {
        try {
          ...
          geojson.features.forEach((feature, idx) => {
            ...
            if (!this._validateWaypointCoordinates(lng, lat)) {
              outOfBoundsWaypoints.push({
                idx,
                name: ((_a2 = feature.properties) == null ? void 0 : _a2.name) || `Waypoint ${idx + 1}`,
          ...
          if (outOfBoundsWaypoints.length > 0 && this.options.maxBounds) {
            const [[west, south], [east, north]] = this.options.maxBounds;
            const waypointList = outOfBoundsWaypoints.map(
              (wp) => `  • ${wp.name}: [${wp.coords[0]}, ${wp.coords[1]}]`
            ).join("\n");
            const confirmed = confirm(
...
Import anyway?`
  ...
  _adjustPanelPosition() {
    ...
    if (attributionControl) {
      ...
      bottomOffset += attrHeight + 10;
    }
    const availableHeight = Math.max(400, viewportHeight - 20 - bottomOffset - 40);
  ...
  _estimateFileSize(bitrate, durationMs, format) {
    var _a, _b, _c;
    const baseSizeMB = bitrate * (durationMs / 1e3) / 8 / 1024;
    const width = ((_a = this._recordingParams) == null ? void 0 : _a.width) || this.options.width || 1920;
    const height = ((_b = this._recordingParams) == null ? void 0 : _b.height) || this.options.height || 1080;
    const fps = ((_c = this._recordingParams) == null ? void 0 : _c.fps) || this.options.fps || 30;
    ...
    if (format === "webm-vp9") {
      ...
      const compressionFactor = isHighQuality ? 1.15 : 0.8;
      return baseSizeMB * compressionFactor;
    }
    return baseSizeMB;
  }
  ...
  _updateProgress(frameCount, totalFrames, bitrate, durationMs, status = "Recording") {
    var _a, _b, _c, _d, _e;
    const statusSpan = (_a = this._progressWidget) == null ? void 0 : _a.querySelector("#ve-progress-status");
    const percentSpan = (_b = this._progressWidget) == null ? void 0 : _b.querySelector("#ve-progress-percent");
    const framesSpan = (_c = this._progressWidget) == null ? void 0 : _c.querySelector("#ve-progress-frames");
    const sizeSpan = (_d = this._progressWidget) == null ? void 0 : _d.querySelector("#ve-progress-size");
    const timeSpan = (_e = this._progressWidget) == null ? void 0 : _e.querySelector("#ve-progress-time");
    if (totalFrames > 0 && this._progressWidget) {
      ...
      if (frameCount > 0 && timeSpan) {
        ...
        const seconds = Math.ceil(estimatedRemainingMs / 1e3);
        if (seconds < 60) {
  ...
  _showFinalStats(stats) {
    ...
    const statusDiv = (_a = this._progressWidget.querySelector(".progress-status")) == null ? void 0 : _a.parentElement;
    const percentDiv = (_b = this._progressWidget.querySelector(".progress-percent")) == null ? void 0 : _b.parentElement;
    const sizeDiv = (_c = this._progressWidget.querySelector(".progress-secondary")) == null ? void 0 : _c.parentElement;
    ...
    if (summaryDiv) {
      ...
      if (speedSpan) {
        ...
        speedSpan.style.color = faster ? "#4CAF50" : "#FF9800";
      }
      if (sizeSpan) {
  ...
  _expandInterface() {
    ...
    formGroups.forEach((group) => {
      ...
      if (!el) return;
      const isConditional = el.id === "ve-resolution-custom-group" || el.id === "ve-speed-custom-group" || el.id === "ve-bitrate-custom-group";
      if (!isConditional) {
        el.style.display = "";
      }
    });
  ...
  async _loadWebmEncoder(sources, width, height, fps, bitrate) {
    ...
    if (!workerUrl || !wasmUrl) {
      throw new Error(
        "WebM encoder files not found. Please deploy the vendor/webm/ directory alongside the plugin. See vendor/README.md for deployment instructions."
      );
    }
  ...
  async _loadWebCodecsVP9Encoder(width, height, fps, bitrate) {
    if (!this._WebCodecsVP9Encoder) {
      const module = await import("./webcodecs-vp9-encoder-SGRGXST7.js");
      this._WebCodecsVP9Encoder = module.WebCodecsVP9Encoder;
    }
    if (!this._WebCodecsVP9Encoder.isSupported()) {
      throw new Error(
        "WebCodecs API not supported in this browser. Use a modern browser or select WebM (VP8) format."
      );
    }
    if (!this._panel) return null;
  ...
  async _detectEncoderSources() {
    ...
    if (this.options.encoderPath) {
      const customPath = this.options.encoderPath.endsWith("/") ? this.options.encoderPath : this.options.encoderPath + "/";
      mp4PathsToTry.push({
        name: "custom path",
  ...
  _readOptionsFromUI() {
    ...
    if (durationSelect) {
      if (durationSelect.value === "custom") {
        ...
        this.options.duration = customInput ? parseFloat(customInput.value) * 1e3 : 3e4;
      } else {
        this.options.duration = parseFloat(durationSelect.value) * 1e3;
      }
    }
    const speedSelect = asSelect(this._panel.querySelector("#ve-speed"));
    if (speedSelect) {
  ...
  _addLoopToAnimation(originalAnimation) {
    return async (map, control) => {
      ...
      if (this.options.loop === "smooth") {
        const returnDuration = Math.min(2e3, this.options.duration * 0.2);
  ...
  _updateExplorationUI() {
    ...
    if (typeof this.options.animation === "string") {
      ...
      const animObj = this.options.animation;
      if (animObj.supportsExploration !== void 0) {
  ...
  _updateAnimationDescription() {
    ...
    const descriptionSpan = descriptionDiv == null ? void 0 : descriptionDiv.querySelector("span");
    if (!descriptionDiv || !descriptionSpan) return;
    const animationName = typeof this.options.animation === "string" ? this.options.animation : null;
  ...
  _checkMapCapabilities() {
    if (!this._panel) return;
    const hasOpenMapTiles = this._map.getSource("openmaptiles") !== void 0;
  ...
  async _testAnimation() {
    ...
    try {
      ...
      this._testProgressTimer = setInterval(() => {
        ...
        const elapsedSeconds = (elapsed / 1e3).toFixed(1);
        const durationSeconds = (duration / 1e3).toFixed(1);
        const percent = Math.min(100, elapsed / duration * 100).toFixed(0);
        const statusSpan = (_a = this._progressWidget) == null ? void 0 : _a.querySelector("#ve-progress-status");
        const percentSpan = (_b = this._progressWidget) == null ? void 0 : _b.querySelector("#ve-progress-percent");
        const timeSpan = (_c = this._progressWidget) == null ? void 0 : _c.querySelector("#ve-progress-time");
  ...
  async _startExploration() {
    ...
    try {
      ...
      const explorationLimitCheckbox = asInput(this._panel.querySelector("#ve-exploration-limit"));
      const isLimitEnabled = (explorationLimitCheckbox == null ? void 0 : explorationLimitCheckbox.checked) || false;
      const originalDuration = this.options.duration;
      if (isLimitEnabled) {
        this.options.duration = this.options.explorationMaxDuration;
        console.log(`🗺️ Exploration limited to ${(this.options.explorationMaxDuration / 1e3).toFixed(0)}s`);
      } else {
        this.options.duration = 999999999;
      ...
      this._exploreProgressTimer = setInterval(() => {
        ...
        if (isLimitEnabled && maxDuration) {
          ...
          const remainingSeconds = Math.max(0, (maxDuration - elapsed) / 1e3).toFixed(1);
          if (percentSpan) percentSpan.textContent = `${percent}% complete`;
          if (timeSpan) timeSpan.textContent = `${elapsedSeconds}s / ${(maxDuration / 1e3).toFixed(0)}s (${remainingSeconds}s left)`;
        } else {
  ...
  async _doRecording() {
    ...
    try {
      ...
      let frameCount = 0;
      let virtualTime = 0;
      const realTimeAdvance = 1e3 / this.options.fps;
      ...
      let recordingDuration = this.options.duration / this.options.speedMultiplier;
      {
        ...
        const targetFrames = Math.floor(recordingDuration / 1e3 * this.options.fps);
      ...
      a.download = `maplibre-video-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 19).replace(/:/g, "-")}.${extension}`;
      a.click();
      const sizeMB = (blob.size / 1024 / 1024).toFixed(2);
      const realElapsedSeconds = ((performance.now() - realStartTime) / 1e3).toFixed(1);
      const videoDurationSeconds = (recordingDuration / 1e3).toFixed(1);
...
__publicField(_VideoExportControl, "DEFAULT_SETTINGS", {
  // Video settings
  "ve-resolution": "auto",
